...
function compactDockerLogs(text) {
...
function looksLikeGitLog(t) { return t.startsWith('commit ') && t.includes('Author:') && t.includes('Date:'); }
...
function looksLikeDockerLogs(t) { return t.split('\n').length > 50; }
function looksLikeKubectl(t) { return /^(NAME\s+READY|NAME\s+STATUS|NAME\s+AGE)/.test(t); }
...
function applyBashPatterns(text) {
    ...
    if (looksLikeDockerLogs(text))
        return compactDockerLogs(text);
...
function compactGrepOutput(text) {
    const lines = text.split('\n').filter(Boolean);
    if (lines.length < 20)
        return text;
    const byFile = {};
    ...
    for (const line of lines) {
        ...
        if (match) {
            ...
            if (byFile[file].length < MAX_GREP_PER_FILE)
                byFile[file].push(content.trim());
            else if (byFile[file].length === MAX_GREP_PER_FILE)
 ...
 */
export function preprocessForTool(text, toolName) {
