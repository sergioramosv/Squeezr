...
function extractInstallSummary(text) {
    const lines = text.split('\n');
    const keep = lines.filter(l => /added \d+/.test(l) ||
        /removed \d+/.test(l) ||
        /Done in/.test(l) ||
...
function compactFileListing(text) {
    const lines = text.split('\n').filter(l => l.trim());
    if (lines.length < 20)
        return text; // not worth compacting small listings
    const byDir = {};
    for (const line of lines) {
...
function looksLikeCargoTest(text) {
    return /test .+\.\.\. (ok|FAILED)/.test(text) || text.includes('running \d+ test');
}
function looksLikeCargoBuild(text) {
...
function looksLikeTsc(text) {
    return /error TS\d+:/.test(text);
}
function looksLikeEslint(text) {
    return /\d+:\d+\s+(error|warning)\s+/.test(text) && text.includes('rule');
}
function looksLikePkgInstall(text) {
