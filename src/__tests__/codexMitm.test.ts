...
function parseWsFrame(buf: Buffer) {
...
describe('processCodexRequest logic', () => {
  ...
  function findToolMessages(input: any[]): Array<{ text: string; field: 'output' | 'content' }> {
...
describe('Codex compression request format', () => {
  ...
  function buildCompressMsg(text: string, model = 'gpt-5.4-mini') {
    return {
  ...
  it('instructions are at top level, not nested', () => {
    const msg = buildCompressMsg('x') as any
    expect(msg.instructions).toBeTruthy()
